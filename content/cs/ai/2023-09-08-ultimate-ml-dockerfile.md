...
*AKTUALIZACE 2024: Tento článek jsem aktualizoval tak, že nyní vychází z obrazu `nvcr.io/nvidia/pytorch`, který dnes používám prakticky vždy díky skvělé podpoře NVIDIA + NCCL + Infiniband. Soubor jsem také zjednodušil a upravil tak, aby pro monitoring GPU používal `gom`.*

Tento článek je určený hlavně kolegům, ale mohl by se hodit i ostatním. Podělím se o Dockerfile, který používám k nastavení svého prostředí pro strojové učení. Vychází z obrazu `pytorch` od NVIDIE, ale přidal jsem do něj pár věcí (aktualizované balíčky Pip, GitHub CLI, Starship prompt, [monitoring GPU](./2023-10-16-gom-gpu-monitor-nvidia-smi-replacement) atd.), které mi přijdou užitečné.

## Nastavení

Zkopírujte níže uvedený `Dockerfile` do nového adresáře. Klidně si přidejte nebo odeberte cokoli chcete — tento post budu nejspíš průběžně aktualizovat podle toho, jak budu své nastavení měnit.
